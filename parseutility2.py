...
def parseFile2(srcFileName):
	...
	try:
		astString = subprocess.check_output("python codesensor2python/build/Main.py " + srcFileName, stderr=subprocess.STDOUT, shell=True)
	except subprocess.CalledProcessError as e:
		print "CodeSensor2-python Error:", e
		astString = ""
	...
	for astLine in astLineList:
		...
		if "function_def" == astLine[0:len("function_def")]:
		...
		elif "identifier" == astLine[0:len("identifier")] and init:
			if funcNameFlag:
				functionInstance.name = astLineSplitted[4].rstrip()
				funcNameFlag = 0
			if paramDeclFlag:
			...
			typeNameFlag = 1
			for name in astLineSplitted[4:]:
				tmpTypeName += (name.rstrip() + ' ')
		
		elif "base_type" == astLine[0:len("base_type")] and init:
			if typeNameFlag:
				functionInstance.dataTypeList.append(tmpTypeName + astLineSplitted[4].rstrip())
				typeNameFlag = 0
				tmpTypeName = ""
...
if __name__ == "__main__":
	...
	functionInstanceList = parseFile2("/home/test/Desktop/discovuler-advanced/test-TnG-tradeoff/case3-arbitrary/short_OLD.vul")
